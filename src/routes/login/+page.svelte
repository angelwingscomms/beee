...
<div class="login-shell rv-field-cloud">
  <div class="login-card">
    <h1 class="rv-d3 login-title">Welcome back.</h1>
    <p class="rv-body login-sub">Sign in to your BEEE account</p>
    ...
    <form onsubmit={handleSubmit} novalidate>
      ...
      {#if apiError}
        <div class="rv-error-text login-error" role="alert">{apiError}</div>
      {/if}

      <button type="submit" class="rv-btn rv-btn--beam felt login-btn" disabled={!allValid || isProcessing}>
    ...
    <a href="/login/google?next={encodeURIComponent(next)}" class="rv-btn rv-btn--ghost felt login-google">
    ...
    <p class="rv-micro login-register-link">NO ACCOUNT? <a href="/register" class="rv-link">REGISTER A CHILD →</a></p>
  </div>
</div>
