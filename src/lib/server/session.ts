...
export async function encode_session(data: { id: string; name?: string; picture?: string; email?: string }, secret: string): Promise<string> {
...
export async function decode_session(c: string | undefined | null, secret: string): Promise<{ user: { id: string; name?: string; picture?: string; email?: string } } | null> {
  if (!c) return null;
